...
The simplest policy is \textbf{random generation}. It samples legal insert, remove, and resize moves from the current clock tree. This policy is not directly guided by timing violations, so many sampled moves may be weak. However, it provides broad exploration and can expose non-obvious area-recovery or resizing opportunities that violation-focused policies may miss.

The most direct timing-driven policy is \textbf{violation path generation}. It scans the data paths, selects paths with negative slack, and prioritizes the most severe violations. For a setup violation, the policy generates moves near the capture flip-flop, because delaying the capture clock increases skew and can improve setup slack. For a hold violation, it generates moves near the launch flip-flop, because delaying the launch clock decreases skew and can improve hold slack. This policy follows the useful-skew formula closely, but its neighborhood can be too narrow when the best repair is not immediately adjacent to the violating endpoint.

To address this limitation, the \textbf{upstream-window policy} expands the search region from a violating endpoint toward the root of the clock tree. Instead of considering only the endpoint edge, it considers several upstream buffers and edges. These moves may affect a larger downstream subtree and can therefore repair several related timing paths at once. The cost is that the candidate set becomes larger and each iteration spends more time evaluating moves.
...
The previous timing-driven policies use different ways to locate promising clock-tree edits. The \textbf{union-pool policy} combines the action sources of violation path, upstream window, and critical endpoint into one candidate pool. To keep the evaluation cost comparable to the individual policies, it uses the same total sampling budget instead of simply increasing the number of evaluated candidates. Its purpose is therefore not to make the candidate set larger, but to make it more diverse under the same budget. Duplicate candidates are removed before evaluation.

Finally, the \textbf{sampled-union-pool policy} is a stochastic variant designed for simulated-annealing-based optimizers. Since simulated annealing evaluates a single proposal and immediately decides whether to accept it, evaluating the full union-pool candidate set at every step would not match its single-move search behavior. Instead, this policy samples only one candidate move from the union pool in each iteration. This keeps the proposal distribution diverse while preserving the low evaluation cost required by simulated annealing.
...
The \textbf{greedy strategy} is the most exploitative choice. It evaluates all candidates in the current set, accepts only moves with positive proxy-score change, and applies the move with the largest improvement. This strategy is stable and easy to interpret: every accepted move immediately improves the current solution according to the proxy score. However, its performance depends heavily on the candidate pool. If the pool does not contain a good improving move, greedy search cannot escape the current local optimum.

The \textbf{two-step strategy} separates timing repair and score recovery into two phases. In the first phase, the solver emphasizes a slack-focused objective, especially setup TNS and WNS. In the second phase, it returns to the full proxy score so that hold timing and area are considered together with setup timing. This strategy follows the intuition that the solver can first create timing margin and then recover area. Its weakness is that the phase boundary is fixed: area recovery may undo previous timing repairs, and timing-focused moves may create too much area overhead for the second phase to repair.

\textbf{Simulated annealing}\cite{kirkpatrick1983optimization} introduces stochastic exploration. At each step, it samples a proposal, evaluates its proxy-score change, and always accepts improving moves. A worsening move may also be accepted with a temperature-controlled probability. At high temperature, the search can move through worse intermediate states and escape local optima; at low temperature, it behaves more like greedy search. The solver still records the best-seen state separately, so temporary degradation does not directly determine the final output.

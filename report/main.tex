...
This two-level decomposition lets us mix and match candidate and accept policies independently, and we evaluate all combinations in Section~\ref{sec:experiments}.
...
The candidate pool merges all focused policies: Violation Path, Upstream Window, Critical Endpoint, plus Remove and Resize candidates.  At each step, one policy family is selected at random (Violation Path 25\%, Upstream Window 20\%, Critical Endpoint 15\%, Remove 20\%, Resize 20\%), and one candidate is sampled from that family for the accept policy to evaluate.

\medskip

\textbf{MILP-inspired Heuristic.}
We also implemented a MILP-inspired heuristic as an alternative approach. However, under the 570-second time budget it achieved less than half the score of the best local search optimizer, so we excluded it from the main comparison and focused on the local search framework described above.
...
\textbf{Unit tests.}
We use Catch2 to test two critical components. First, \texttt{test\_evaluation.cpp} constructs a small clock tree with known delays, computes timing metrics by hand, and checks that \texttt{evaluate()} produces exactly matching TNS, WNS, and area values.  It also verifies that the \texttt{score()} formula correctly applies the $\alpha$, $\beta$, $\gamma$ weights.  Second, \texttt{test\_clock\_tree.cpp} verifies that every edit type (insert, resize, remove) is fully reversible: after \texttt{undo()}, the parent--child connectivity and node alive flags are restored to their original state.  It also confirms that original contest nodes cannot be removed through the optimizer edit API.

\textbf{Ground-truth re-evaluation.}
The solver maintains an incremental \texttt{TimingState} for fast per-step scoring, and a separate full \texttt{evaluate()} that recomputes all clock arrivals and slacks from scratch.  \texttt{evaluate()} is called at the start (to record the baseline) and at the end (to record the final metrics), serving as the authoritative check on correctness.

\textbf{Structural legality.}
The clock tree API enforces that only buffers inserted by the optimizer (\texttt{NodeOrigin::Inserted}) can be removed; attempts to remove original contest nodes return an empty edit.  The output file is written atomically via a temp-file rename, so the file on disk is always a complete, valid tree structure.

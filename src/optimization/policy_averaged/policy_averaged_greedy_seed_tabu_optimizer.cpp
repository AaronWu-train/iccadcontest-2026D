/**
 * @file policy_averaged_greedy_seed_tabu_optimizer.cpp
...
void append_critical_endpoint_candidates(const ClockTree& clock_tree, const TimingState& timing,
    ...
    for (const auto& [ff_id, value] : criticality) {
        ranked.emplace_back(ff_id, value);
    }
...
std::vector<CandidateScore> score_candidates(
    ...
    const Metrics& baseline_metrics, std::vector<CandidateMove> candidates, std::size_t limit,
    ...
    for (const auto& candidate : candidates) {
        ...
        scored.push_back(CandidateScore{candidate, candidate_score, candidate_score - before_score});
...
void add_ranked_policy_scores(std::unordered_map<std::string, PolicyAggregate>& aggregate,
                              const std::vector<CandidateScore>& scored) {
    const std::size_t count = scored.size();
    if (count == 0) {
        return;
    }
    for (std::size_t rank = 0; rank < count; ++rank) {
        if (scored[rank].delta <= 0.0) {
            continue;
        }
        const std::string key = move_key(scored[rank].move);
        auto& entry = aggregate[key];
        if (entry.votes == 0) {
            entry.move = scored[rank].move;
        }
        const double normalized_rank =
            count == 1 ? 1.0 : 1.0 - static_cast<double>(rank) / static_cast<double>(count - 1);
        entry.rank_sum += normalized_rank;
        entry.best_delta = std::max(entry.best_delta, scored[rank].delta);
        entry.best_score = std::max(entry.best_score, scored[rank].score);
        ++entry.votes;
    }
}

bool choose_policy_averaged_seed_move(
    ClockTree& clock_tree, TimingState& timing, const BufferLibrary& buffer_library,
    const Metrics& baseline_metrics, const PolicyAveragedGreedySeedTabuConfig& config,
    const std::chrono::steady_clock::time_point& deadline, CandidateMove& best_move,
    double& best_delta) {
    std::unordered_map<std::string, PolicyAggregate> aggregate;

    std::vector<CandidateMove> violation_candidates;
    append_violation_path_candidates(clock_tree, timing, config.violation_sample_limit,
                                     violation_candidates);
    add_ranked_policy_scores(
        aggregate, score_candidates(clock_tree, timing, buffer_library, baseline_metrics,
                                    std::move(violation_candidates), config.seed_candidate_limit,
                                    deadline));

    std::vector<CandidateMove> critical_candidates;
    append_critical_endpoint_candidates(clock_tree, timing, config.critical_endpoint_limit,
                                        critical_candidates);
    add_ranked_policy_scores(
        aggregate, score_candidates(clock_tree, timing, buffer_library, baseline_metrics,
                                    std::move(critical_candidates), config.seed_candidate_limit,
                                    deadline));

    std::vector<CandidateMove> upstream_candidates;
    append_upstream_window_candidates(clock_tree, timing, config.violation_sample_limit,
                                      config.upstream_window_depth, upstream_candidates);
    add_ranked_policy_scores(
        aggregate, score_candidates(clock_tree, timing, buffer_library, baseline_metrics,
                                    std::move(upstream_candidates), config.seed_candidate_limit,
                                    deadline));

    bool found = false;
    double best_average = -std::numeric_limits<double>::infinity();
    double best_value = -std::numeric_limits<double>::infinity();
    for (const auto& [key, candidate] : aggregate) {
        (void)key;
        if (candidate.best_delta <= config.min_seed_delta || candidate.votes == 0) {
            continue;
        }
        const double average_rank = candidate.rank_sum / static_cast<double>(candidate.votes);
        const double consensus_multiplier =
            1.0 + config.consensus_bonus * static_cast<double>(candidate.votes - 1);
        const double value = average_rank * consensus_multiplier + 1e-6 * candidate.best_delta;
        if (!found || value > best_value ||
            (value == best_value && candidate.best_delta > best_delta)) {
            found = true;
            best_move = candidate.move;
            best_delta = candidate.best_delta;
            best_average = average_rank;
            best_value = value;
        }
    }
    (void)best_average;
...
void remember_tabu_move(std::unordered_map<std::string, std::size_t>& tabu_until,
    ...
    const std::string key = move_key(move);
...
bool choose_tabu_move(ClockTree& clock_tree, TimingState& timing,
                      const BufferLibrary& buffer_library, const Metrics& baseline_metrics,
                      const BestRunState& best_state,
                      const PolicyAveragedGreedySeedTabuConfig& config,
                      ...
                      CandidateMove& best_move, double& best_candidate_score) {
    std::vector<CandidateMove> candidates;
    append_violation_path_candidates(clock_tree, timing, config.violation_sample_limit,
                                     candidates);
    append_critical_endpoint_candidates(clock_tree, timing, config.critical_endpoint_limit,
    ...
    bool found = false;
    best_candidate_score = -std::numeric_limits<double>::infinity();
    for (const auto& candidate : candidates) {
        if (std::chrono::steady_clock::now() >= deadline) {
            break;
        }
...
void PolicyAveragedGreedySeedTabuOptimizer::run(ClockTree& clock_tree,
    ...
    const PolicyAveragedGreedySeedTabuConfig config =
        policy_averaged_greedy_seed_tabu_config_from_sources(context.optimizer_config);
    ...
    record_trace(context, clock_tree,
                 make_event(start_time, 0, "baseline", "kept", timing, baseline_metrics,
                            ...
                            "policy_averaged_seed_tabu"),
                 true);

    for (std::size_t seed_step = 0;
         seed_step < config.seed_steps && std::chrono::steady_clock::now() < deadline;
         ++seed_step) {
        CandidateMove seed_move;
        double seed_delta = 0.0;
        if (!choose_policy_averaged_seed_move(clock_tree, timing, buffer_library, baseline_metrics,
                                              config, deadline, seed_move, seed_delta)) {
            break;
        }
        const double before_score = current_score;
        if (!apply_candidate(clock_tree, timing, buffer_library, seed_move)) {
            ++rejected_moves;
            break;
        }
        current_score = timing.score(baseline_metrics);
        ++accepted_moves;
        remember_tabu_move(tabu_until, tabu_queue, seed_move, accepted_moves, config.tenure);
        ...
        record_trace(context, best_updated ? best_state.tree : clock_tree,
                     make_event(start_time, accepted_moves, "policy_average_seed",
                                best_updated ? "best_update" : "accepted", timing,
                                baseline_metrics, best_state, accepted_moves, rejected_moves,
                                "policy_average", current_score - before_score),
                     best_updated);
    }

    for (std::size_t tabu_step = 0;
         tabu_step < config.tabu_steps && std::chrono::steady_clock::now() < deadline;
         ++tabu_step) {
        CandidateMove best_move;
        double best_candidate_score = -std::numeric_limits<double>::infinity();
        if (!choose_tabu_move(clock_tree, timing, buffer_library, baseline_metrics, best_state,
                              config, tabu_until, deadline, accepted_moves, best_move,
        ...
        current_score = timing.score(baseline_metrics);
        ++accepted_moves;
        remember_tabu_move(tabu_until, tabu_queue, best_move, accepted_moves, config.tenure);

        const bool best_updated =
            maybe_update_best(clock_tree, timing, baseline_metrics, best_state);
        context.maybe_checkpoint(best_state.tree, accepted_moves);
        record_trace(context, best_updated ? best_state.tree : clock_tree,
                     make_event(start_time, accepted_moves, "tabu_polish",
                                best_updated ? "best_update" : "accepted", timing,
                                baseline_metrics, best_state, accepted_moves, rejected_moves,
                                "policy_seed_then_tabu", current_score - before_score),
                     best_updated);
    }
    ...
    debug.log([&](std::ostream& os) {
        os << "PolicyAveragedGreedySeedTabuOptimizer: accepted = " << accepted_moves
